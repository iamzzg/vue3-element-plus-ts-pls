...
export function getAppEnvConfig() {
  ...
  const { VITE_GLOBAL_APP_TITLE, VITE_GLOBAL_BASIC_API, VITE_GLOBAL_WINDOW_CONFIG_KEY } = ENV

  if (!/^[a-zA-Z_]*$/.test(VITE_GLOBAL_WINDOW_CONFIG_KEY)) {
    warn(
      `VITE_GLOBAL_WINDOW_CONFIG_KEY 变量只能包含 数字/字母/下划线_,请重新在.env文件中修改并重新运行程序`
    )
  }

  return { VITE_GLOBAL_APP_TITLE, VITE_GLOBAL_BASIC_API, VITE_GLOBAL_WINDOW_CONFIG_KEY }
}
